...
| **Components** | `components/` | Reusable UI primitives (`primitives/`), table wrappers (`table/`), custom chart modules (`ui/charts/` including `uPlot` setups, micro charts, and uplot helpers), dashboard layouts, `ui/PanelCard.tsx`, and domain drawers (`ui/drawers/ServiceDetailDrawer`). Health indicators: `ui/data-display/status/` (`StatusDot`, `StatusPill`, and the canonical `HealthStatus` vocabulary) — features translate their local status vocab to `HealthStatus` at the call site. Data tables: new tables use `ui/data-display/DataTable` (TanStack-table wrapper: columns, `meta.align`, row-click via `onRow`, sticky header, virtualization). It does not yet do column sorting or pagination — add those to DataTable rather than hand-rolling `<table>`. Compact list layouts inside drawers may stay bespoke. |
