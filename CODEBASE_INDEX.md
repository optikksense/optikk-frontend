...
| Traces | `src/features/traces/` | Trace explorer, detail, comparison. Trace detail page uses Datadog-parity layout: full-width viz (Waterfall + Flame Graph; waterfall renders event dots on bars at event timestamps) with non-modal resizable right `SpanDrawer` for span detail (Info / Logs / Events / Links / Infra tabs, hide-when-empty). Info tab includes a "Where this happens" ancestor chain + "Timing" KV grid (now with a `span-detail/SelfChildBar` stacked self-vs-child % bar); drawer header shows a "critical path" pill when the selected span is on the critical path. Composition root at `pages/TraceDetailPage/components/TraceDetailLayout.tsx`. **Summary strip** (in `pages/TraceDetailPage/index.tsx`, between `KPIStrip` and `ServiceStrip`): `components/ServiceTimeBar.tsx` (stacked horizontal bar of each service's share of total span time, from `tracesService.getServiceMap` → `ServiceMapResponse.nodes` `{service,total_ms,span_count,error_count}`) + `components/HotSpansStrip.tsx` (top-3 spans by self-time, click-to-open; computed in `utils/hotSpans.ts` from flamegraph frames' `self_time_ms`). **Errors viz tab** (`components/ErrorsTab.tsx`) now renders `components/TraceErrorSummary.tsx` (exception groups from `tracesService.getTraceErrors` → `TraceErrorGroup[]` `{exception_type,count,spans[]}`, expandable to offending spans) above the existing flat error-span list. Detail data hooks: composition in `pages/TraceDetailPage/hooks/useTraceDetailState.ts` fans out over sibling hooks `hooks/{useTraceServiceMap (eager), useTraceErrors (gated on errors tab), useTraceHotSpans (eager flamegraph-frames query, key `trace-flamegraph-frames`)}` alongside `useTraceDetailData`/`useTraceDetailEnhanced`/`useTraceFlamegraph`; surfaced via `useTraceDetailPage` (`serviceMapNodes`, `hotSpans`, `layoutProps.errorGroups`). **Trace list quick-look**: `pages/TracesExplorerPage/TraceQuickLook.tsx` is a right-rail preview (360px inline grid column, mirrors logs `LogDetailPanel`) for the focused trace row — status/operation/duration/span+error counts from the `/traces/query` row, services from `service_set`, slowest spans lazily from `getTraceSpans` (shares `trace-spans` query key). Row single-click sets `focusedTraceId` (highlights via `ResultsArea selectedId`) and opens the preview; preview's "Open trace" button navigates to detail. State in `store/tracesStore.ts` (persists `visualizationTab`, `spanDetailTab`, `drawerWidthPx`); URL holds `?span=<id>`. Hotkeys: `/` filter, `j`/`k` or ↑/↓ navigate spans, `c` copy trace id, `e` cycle errors, `1`/`2` switch viz, `[`/`]` resize drawer, `Esc` close. |
