...
export default function DemoLogsList({ limit }: DemoLogsListProps) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-muted font-mono text-[12px]">
      {rows.map((row, i) => (
        <div
          key={i}
          className="grid items-start gap-3 border-border border-b px-3 py-1.5 last:border-b-0"
          style={{ gridTemplateColumns: "72px 64px 120px 1fr" }}
        >
          <span className="text-foreground-muted">{formatTs(row.tsMs)}</span>
          <span className="font-semibold" style={{ color: LEVEL_TONE[row.level] }}>
            {row.level}
          </span>
          <span className="overflow-hidden text-ellipsis whitespace-nowrap text-foreground-secondary">
            {row.service}
          </span>
