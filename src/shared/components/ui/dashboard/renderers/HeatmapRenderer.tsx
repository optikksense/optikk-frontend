...
export function HeatmapRenderer({
  ...
  const xValues = useMemo(
    () =>
      Array.from(new Set(rows.map((r: Record<string, unknown>) => String(r[xKey] ?? "")))).slice(
        0,
        20
      ),
    [rows, xKey]
  ...
  const { yValues, lookup, maxVal, yOverflow } = useMemo(() => {
    const Y_CAP = 50;
    const allYVals = Array.from(
      new Set(rows.map((r: Record<string, unknown>) => String(r[yKey] ?? "")))
    );
