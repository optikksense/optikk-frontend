...
function FacetGroupComponent({
  ...
  return (
    <section className="flex flex-col gap-1 border-border border-b px-2 py-2">
      <button
        type="button"
        onClick={() => setOpen((prev) => !prev)}
        className="flex w-full items-center gap-1 font-semibold text-[11px] text-foreground-secondary uppercase tracking-wider"
        aria-expanded={open}
      >
        <Icon size={12} />
        {group.label}
