...
function DetailTabsComponent({ tabs, value, onChange }: Props) {
  return (
    <Tabs.Root value={value} onValueChange={onChange} className="flex min-h-0 flex-1 flex-col">
      <Tabs.List className="flex shrink-0 items-center gap-2 border-[var(--border-color)] border-b px-3">
        {tabs.map((tab) => (
          <Tabs.Trigger
            key={tab.key}
            value={tab.key}
            className="border-transparent border-b-2 px-1 py-2 font-medium text-[12px] text-[var(--text-secondary)] transition-colors data-[state=active]:border-[var(--accent)] data-[state=active]:text-[var(--text-primary)]"
          >
            {tab.label}
          </Tabs.Trigger>
        ))}
