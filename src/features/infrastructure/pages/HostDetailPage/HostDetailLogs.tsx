...
import { buildLogsHubHref, hostEqualsFilter } from "@shared/observability/deepLinks";
import { formatTime } from "@shared/utils/formatters";

const PREVIEW_LIMIT = 25;

interface HostDetailLogsProps {
...
export function HostDetailLogs({ host }: HostDetailLogsProps) {
  const navigate = useNavigate();
  const { getTimeRange } = useTimeRange();

  const logsQ = useTimeRangeQuery(`host-detail.logs.${host}`, (_tenant, s, e) =>
    queryLogs({
      startTime: Number(s),
      endTime: Number(e),
      filters: [{ field: "host", op: "eq", value: host }],
      limit: PREVIEW_LIMIT,
    })
  );
    ...
    <section className="rounded-md border border-border bg-card p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-[11px] text-foreground-muted">
            Latest {PREVIEW_LIMIT} log lines from this host in the current time range
          </div>
        </div>
      ...
      {logs.length === 0 ? (
        <div className="grid h-[80px] place-items-center text-[12px] text-foreground-muted">
          {logsQ.isPending ? "Loading logs…" : "No logs from this host in the current time range."}
        </div>
      ) : (
