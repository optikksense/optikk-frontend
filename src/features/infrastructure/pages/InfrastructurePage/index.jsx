...
export default function InfrastructurePage() {
  ...
  const stats = useMemo(() => {
    ...
    const uniqueHosts = new Set(
      data.map((item) => item.host ?? item.host_name ?? 'unknown').filter(Boolean)
    ).size;
    const uniquePods = new Set(
      data.map((item) => item.pod ?? item.pod_name).filter((pod) => pod && pod.trim() !== '')
    ).size;
  ...
  const tableData = useMemo(() => {
    ...
    return data.map((item, index) => ({
      ...item,
      host: item.host ?? item.host_name ?? 'unknown',
      pod: item.pod ?? item.pod_name ?? '',
      container: item.container ?? item.container_name ?? '',
