...
function Row({ row }: { row: DeploymentImpactRow }) {
  return (
    <tr className="h-10 border-border border-t">
      <td className={COL_BODY}>
        <div className="flex items-baseline gap-2">
          <span className="font-mono text-[12px] text-foreground">{row.version}</span>
          {row.is_baseline && (
            <span className="rounded bg-[var(--bg-elevated,rgba(255,255,255,0.06))] px-1.5 py-0.5 text-[10px] text-foreground-muted uppercase tracking-wide">
              baseline
            </span>
          )}
        </div>
      ...
      <td className={COL_BODY}>
        <span className="text-foreground-muted">{relativeTimeFromIso(row.deployed_at)}</span>
      </td>
