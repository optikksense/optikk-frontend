...
export default function AiPerformanceTab({
  ...
  const tableColumns = [
    ...
    { title: 'Tokens/sec', dataIndex: 'avg_tokens_per_sec', key: 'avg_tokens_per_sec', render: (value: any) => { const normalized = n(value); return normalized == null ? naSpan() : <span style={{ fontWeight: 600, color: '#9E77ED' }}>{normalized.toFixed(1)}</span>; }, sorter: (a: any, b: any) => (n(a.avg_tokens_per_sec) ?? -1) - (n(b.avg_tokens_per_sec) ?? -1), align: 'right' as const },
    { title: 'Timeouts', dataIndex: 'timeout_count', key: 'timeout_count', render: (value: any) => { const normalized = n(value) ?? 0; return <span style={{ color: normalized > 0 ? '#F04438' : '#73C991', fontWeight: 600 }}>{formatNumber(normalized)}</span>; }, sorter: (a: any, b: any) => (n(a.timeout_count) ?? 0) - (n(b.timeout_count) ?? 0), align: 'right' as const },
